...
class PMMs(nn.Module):
    ...
    def forward(self, support_feature, query_feature):
        prototypes, mu_f = self.generate_prototype(support_feature)
    ...
    def generate_prototype(self, feature):
        ...
        # foreground
        mu_f = self.get_prototype(feature)
        mu_ = []
    ...
    def discriminative_model(self, query_feature, mu_f):

        mu = mu_f.permute(0, 2, 1)

        b, c, h, w = query_feature['res4'].size()
        x = query_feature['res4'].view(b, c, h * w)  # b * c * n
        with torch.no_grad():
        ...
        Prob_map = torch.sum(P[:, 0:self.num_pro], dim=1).unsqueeze(dim=1)
